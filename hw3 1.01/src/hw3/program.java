				...
				if (s1.All_state.get(j).getstate().equals(state)){
					...
					for(int i = 0; i <result.length;i++){
						...
						//	System.out.println("The Index I in the sorted list is "+ i);
		    				int temp1= i-1;
		    				int temp2 = i+1; 
		    				double gap1=Math.abs(result[temp1]-result[i]) ;
		    				double gap2=Math.abs(result[temp2]-result[i]) ;
		    				...
		    				if(temp2>=result.length){
		    					...
		    					for (int k =0; k<result.length;k++){
		    		    			if (s1.All_state.get(k).per_change()==result[-2]){
		    		    				System.out.println("The  most similar state  in terms of percent change in population is :  ");
		    		    				System.out.printf("%-30.30s  %-30.30s%n", s1.All_state.get(k).getstate(),defaultFormat.format(result[-2])) ;
		    		    			}
		    		    		}	
		    				}
