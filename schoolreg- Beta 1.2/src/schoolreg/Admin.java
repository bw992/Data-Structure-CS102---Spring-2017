	 ...
	 // if not return false 
		...
		public  void Edit_course(String course_id,int sec){
			if (Course.get_Course(course_id,sec) != null){
				Course.edit_course(course_id,sec);
			}
		...
		public void write_file_of_list() throws IOException{
			...
			System.out.println("The list is ongoing");
			String str = Course.list_if_class_full();
			writer.write(str);
			writer.close();
			System.out.println("written completed");
			System.out.println("The output file is in the home directory");
			System.out.println("File name: output.txt");
		...
		public void view_names_in_class(String id){
			...
			Course.find_studentnames_inclass(id);
		}
		...
		public void student_class(String fullname){
			...
			Course.find_student_all_class(fullname);
		}
		//6 Sort courses based on current number of student 
		
		public void sort_courses(){ 
		Collections.sort(Course.All_course, Course.StuRollno);
		for (int i = 0; i < Course.All_course.size(); i++) {
			 System.out.println("Course Name:" + Course.All_course.get(i).get_Course_Name());
		     System.out.println("Course id:" +Course.All_course.get(i).get_Course_id());
		     System.out.println("Course cur-stu:" +Course.All_course.get(i).get_Current_Students());
		     ...
		     System.out.println("-----------------------------------------");}
				...
				/*Collections.sort(Course.All_course);
				for (int i = 0; i < Course.All_course.size(); i++) {
					 System.out.println("Course Name:" + Course.All_course.get(i).get_Course_Name());
				     System.out.println("Course id:" +Course.All_course.get(i).get_Course_id());
				     System.out.println("Course cur-stu:" +Course.All_course.get(i).get_Current_Students());
