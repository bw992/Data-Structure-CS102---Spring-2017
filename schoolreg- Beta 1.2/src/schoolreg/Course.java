...
public class Course implements Serializable,Comparable<Course>{
	...
	public static void set_all_course(ArrayList x){
		Course.All_course = x;
	}
	//course id 
	...
	public static String get_Course(String coursename){
		for(int i =0;i<All_course.size();i++){
		...
		for (int i =0;i<All_course.size();i++){
			if (course_name.equals(All_course.get(i).Course_id)& sec_number == All_course.get(i).Course_Section_Number){
				...
				case 2:
					...
					return;
				case 3:
					System.out.println("Enter new max student number"); 
					int s1 = input.nextInt(); 
					input.nextLine();
					All_course.get(i).set_Course_maximum_students(s1);
					return;
				case 4:
					System.out.println("Enter new current student number"); 
					int s2 = input.nextInt(); 
					input.nextLine();
					All_course.get(i).set_Course_current_students(s2);
					return;
				case 5:
					System.out.println("Enter new list of students"); 
					ArrayList<String> myList = new ArrayList<String>();
					String new_list_stu = input.nextLine(); 
					//All_course.get(i).List_Of_Names.add(new_list_stu);
					All_course.get(i).Current_Students+=1;
					All_course.get(i).set_Course_add_student(new_list_stu);
					return;
				case 6:
	...
	/*public static ArrayList<Course> sort_list(){
		...
		for(int x =0; x<All_course.size();x++){
	        ...
	        for(int y = 0; y < All_course.size()-1; y++){
		           if(All_course.get(y+1).Current_Students < All_course.get(y).Current_Students) {
	 ...
	 public static Comparator<Course> StuRollno = new Comparator<Course>() {

			public int compare(Course s1, Course s2) {

			   int rollno1 = s1.get_Current_Students();
			   int rollno2 = s2.get_Current_Students();
	...
	public static void check_if_class_not_full(){
		...
		for (int i =0; i<full_course.size();i++){
			System.out.println(full_course.get(i).Course_Name );
			System.out.println(full_course.get(i).Course_id );
			System.out.println("Course current student number:" +full_course.get(i).Current_Students );
			System.out.println("Course Max student number:" +full_course.get(i).Maximum_Students);
			System.out.println("-----------------------------------------");
